...
then
    (
        get_resource_version Compurene/jellyfin-web-jmp
...
then
    if [[ "$(get_resource_version Compurene/jellyfin-web-jmp)" != "$(cat .last_wc_version)" ]]
    then
        update_web_client="yes"
    fi
fi
...
then
    ...
    wc_version=$(get_resource_version Compurene/jellyfin-web-jmp)
    download_compat dist.zip "https://github.com/Compurene/jellyfin-web-jmp/releases/download/$wc_version/dist.zip" "wc"
    if [[ "$DOWNLOAD_ONLY" != "1" ]]
    then
